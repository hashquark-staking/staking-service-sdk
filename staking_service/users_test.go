...
func TestUserProcess(t *testing.T) {
	...
	//createResult, code, msg, err := stakingService.CreateUser("", []string{"0x2B3779A253dB55B98eCED3EF427992740C17db17"})
	//require.Nil(t, err)
	...
	//require.Equal(t, "ok", msg)
	//
	detailsResult, code, msg, err := stakingService.GetUserDetails("mkx")
	fmt.Println(detailsResult)
	require.Nil(t, err)
	require.Equal(t, uint(0), code)
